...
/*
 * Map used for global cost accounting. Can be extended to support LLCs.
 */
struct {
 ...
 */
static void initialize_cost_layer(struct cost *costc,
				  u32 layer_id,
				  bool overflow,
				  s64 capacity)
{
...
static u32 preferred_cost(struct cost *costc)
{
	...
	bpf_for(id, 0, nr_layers) {
		...
		if (costc->budget[layer_id] > max_budget) {
			max_budget = *MEMBER_VPTR(*costc, .budget[layer_id]);
			max_layer = layer_id;
		}
	}
...
s64 acquire_budget(struct cost *costc, u32 layer_id, s64 amount)
{
	...
	__sync_fetch_and_sub(MEMBER_VPTR(*parent_cost, .budget[layer_id]),
			     amount);
...
static int record_cpu_cost(struct cost *costc, u32 layer_id, s64 amount)
{
	...
	__sync_fetch_and_sub(MEMBER_VPTR(*costc, .budget[layer_id]), amount);
...
static void initialize_budgets(u64 refresh_intvl_ns)
{
	...
	struct cost *costc;
	int layer_id;
	u64 layer_weight, layer_weight_dur, layer_weight_sum = 0;
	u64 layer_slice_dur = 0;
	s32 cpu;
	u32 global = 0;
	...
	bpf_for(layer_id, 0, nr_layers) {
		...
		layer_slice_dur += layer_slice_ns;
	}
	...
	bpf_for(layer_id, 0, nr_layers) {
		...
		layer_weight_dur = (layer->weight * ((u64)refresh_intvl_ns * nr_possible_cpus)) /
				    layer_weight_sum;
		initialize_cost_layer(costc, layer_id, false,
		...
		bpf_for(cpu, 0, nr_possible_cpus) {
			...
			layer_weight_dur = (layer->weight * layer_slice_ns * refresh_intvl_ns) /
					    layer_weight_sum;
			initialize_cost_layer(costc, layer_id, true,
					      (s64)layer_weight_dur);
			if (cpu == 0)
